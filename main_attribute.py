...
class Ex(Ui_Form):
    ...
    def __init__(self, opt):
        ...
        self.get_reference_image = False
        ...
        self.refer_scene = GTScene(self)
        self.referenceView.setScene(self.refer_scene)
        self.referenceView.setAlignment(Qt.AlignCenter)
        self.referenceView.setVerticalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        self.referenceView.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
    ...
    def pre_computing_distance(self):
        self.pre_lighting_distance = [self.pre_lighting[i] - self.array_light for i in range(len(self.lighting_order))]
        self.pre_attr_distance = [self.R_attr_current_list[i] - self.attr_current_list[i] for i in
                                  range(len(self.attr_order))]
    ...
    def update_GT_scene_image(self):
        ...
        # self.attr_current_dic = {j: self.attr_current[i][0] for i, j in enumerate(self.attr_order)}
        ...
        qim = QImage(self.GAN_image.data, self.GAN_image.shape[1], self.GAN_image.shape[0], self.GAN_image.strides[0],
                     QImage.Format_RGB888)
        self.original_GAN_image = self.GAN_image.copy()
        ...
        if self.get_reference_image:
            self.pre_computing_distance()
    ...
    def update_Reference_scene_image(self):
        ...
        self.R_w_current = self.all_w[self.scene.right_pickedImageIndex].copy()
        self.R_attr_current = self.all_attr[self.scene.right_pickedImageIndex].copy()
        self.R_light_current = self.all_lights[self.scene.right_pickedImageIndex].copy()

        self.R_attr_current_list = [self.R_attr_current[i][0] for i in range(len(self.attr_order))]

        self.pre_lighting = [torch.from_numpy(self.R_light_current).type(torch.FloatTensor).cuda()]

        self.pre_computing_distance()

        self.R_GAN_image = self.model.generate_im_from_w_space(self.R_w_current)[0]

        qim = QImage(self.R_GAN_image.data, self.R_GAN_image.shape[1], self.R_GAN_image.shape[0],
                     self.R_GAN_image.strides[0],
                     QImage.Format_RGB888)

        showedImagePixmap = QPixmap.fromImage(qim)
        # showedImagePixmap = showedImagePixmap.scaled(QSize(256, 256), Qt.IgnoreAspectRatio)
        self.refer_scene.reset()
        if len(self.refer_scene.items()) > 0:
            self.refer_scene.reset_items()

        self.refer_scene.addPixmap(showedImagePixmap)
        self.get_reference_image = True
